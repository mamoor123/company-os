...
router.post("/", authMiddleware, upload.single("file"), async (req, res) => {
	...
	const result = await db
		.prepare(
			"INSERT INTO uploads (filename, original_name, mime_type, size, path, uploaded_by, task_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
		)
		.run(
			req.file.filename,
...
router.get("/task/:taskId", authMiddleware, async (req, res) => {
	res.json(
		await db
			.prepare(
				"SELECT u.*, usr.name as uploader_name FROM uploads u LEFT JOIN users usr ON u.uploaded_by = usr.id WHERE u.task_id = ? ORDER BY u.created_at DESC",
			)
