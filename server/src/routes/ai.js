...
router.post("/execute/:taskId", authMiddleware, async (req, res) => {
	try {
		const result = await executeTask(parseInt(req.params.taskId, 10));
...
router.post("/delegate", authMiddleware, async (req, res) => {
	try {
		...
		if (!fromAgentId || !toAgentId || !message)
			return res
				.status(400)
				.json({ error: "fromAgentId, toAgentId, and message are required" });
...
router.get("/messages/:channel", authMiddleware, async (req, res) => {
	...
	const messages = await db
		.prepare(
			`SELECT m.*, CASE WHEN m.sender_type = 'user' THEN u.name ELSE a.name END as sender_name FROM messages m LEFT JOIN users u ON m.sender_type = 'user' AND m.sender_id = u.id LEFT JOIN agents a ON m.sender_type = 'agent' AND m.sender_id = a.id WHERE m.channel = ? ORDER BY m.created_at DESC LIMIT ?`,
		)
...
router.get("/channels", authMiddleware, async (_req, res) => {
	const channels = await db
		.prepare(
			"SELECT DISTINCT channel, COUNT(*) as message_count, MAX(created_at) as last_activity FROM messages GROUP BY channel ORDER BY last_activity DESC",
		)
		.all();
