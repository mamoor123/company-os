...
router.get("/:id", authMiddleware, async (req, res) => {
	const task = await db
		.prepare(`SELECT t.*, d.name as department_name, d.icon as department_icon, d.color as department_color,
...
router.post(
	...
	async (req, res) => {
		...
		const result = await db
			.prepare(`INSERT INTO tasks (title, description, priority, department_id, assigned_to, assigned_agent_id, created_by, due_date)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
			.run(
				title,
				description || "",
		...
		const task = await db
			.prepare(
				"SELECT t.*, d.name as department_name, u.name as assignee_name FROM tasks t LEFT JOIN departments d ON t.department_id = d.id LEFT JOIN users u ON t.assigned_to = u.id WHERE t.id = ?",
			)
...
router.post("/:id/comments", authMiddleware, async (req, res) => {
	...
	await db
		.prepare("UPDATE tasks SET updated_at = CURRENT_TIMESTAMP WHERE id = ?")
		.run(req.params.id);
	const comments = await db
		.prepare(
			"SELECT c.*, u.name as user_name FROM task_comments c LEFT JOIN users u ON c.user_id = u.id WHERE c.task_id = ? ORDER BY c.created_at",
		)
		.all(req.params.id);
	const task = await db
		.prepare("SELECT * FROM tasks WHERE id = ?")
...
router.get("/:id/comments", authMiddleware, async (req, res) => {
	const comments = await db
		.prepare(
			"SELECT c.*, u.name as user_name, a.name as agent_name FROM task_comments c LEFT JOIN users u ON c.user_id = u.id LEFT JOIN agents a ON c.agent_id = a.id WHERE c.task_id = ? ORDER BY c.created_at",
		)
		.all(req.params.id);
	res.json(comments);
});
