...
function getBackoffDelay(retryCount) {
	return Math.min(BACKOFF_BASE_MS * BACKOFF_MULTIPLIER ** retryCount, 300_000);
}
...
async function executeTaskAsync(task, isRetry) {
	...
	console.log(
		`🤖 ${isRetry ? "Retrying" : "Executing"} task #${task.id}${isRetry ? ` (attempt ${retryCount + 1}/${task.max_retries})` : ""}: "${task.title}" via ${task.agent_name}`,
	);
	...
	try {
		...
		await db
			.prepare(
				"UPDATE tasks SET retry_count = 0, last_error = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
			)
			.run(task.id);
		if (task.created_by)
		...
		if (newRetryCount >= maxRetries) {
			...
			await db
				.prepare(
					"UPDATE tasks SET status = 'blocked', retry_count = ?, last_error = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
				)
			...
			console.warn(
				`⚠️ Task #${task.id} failed (attempt ${newRetryCount}/${maxRetries}), retrying in ${backoffSec}s: ${err.message}`,
			);
			await db
				.prepare(
					"UPDATE tasks SET retry_count = ?, last_error = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
				)
				.run(newRetryCount, err.message, task.id);
			if (task.created_by)
				notificationService.notify({
