			...
			const defaults = [
				{
					...
					description: "Notify manager on urgent task creation",
				...
				},
				{
					...
					description: "Escalate tasks past due date",
					trigger: "schedule_daily",
					conditions: JSON.stringify([
						{ field: "due_date", operator: "past_due" },
					]),
					actions: JSON.stringify([
						{ type: "notify", target: "admin", message: "Task overdue" },
				...
				},
				{
					...
					description: "Welcome message on registration",
					...
					enabled: true,
					runs: 1,
					last_run: new Date(Date.now() - 36000000).toISOString(),
				},
			];
			const insert = db.prepare(
				'INSERT INTO workflows (name, description, "trigger", conditions, actions, enabled, runs, last_run) VALUES (@name, @description, @trigger, @conditions, @actions, @enabled, @runs, @last_run)',
			);
...
async function createWorkflow(data, userId) {
	const result = await db
		.prepare(
			'INSERT INTO workflows (name, description, "trigger", conditions, actions, enabled, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
		)
		.run(
			data.name,
			data.description || "",
...
async function toggleWorkflow(id) {
	await db
		.prepare(
			"UPDATE workflows SET enabled = NOT enabled, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		)
		.run(id);
	const row = await db.prepare("SELECT * FROM workflows WHERE id = ?").get(id);
	return rowToWorkflow(row);
	...
	for (const action of actions) {
		try {
			...
			message = message.replace(
				/\{\{(\w+)\.(\w+)\}\}/g,
				(_, obj, field) => context[obj]?.[field] || `[${obj}.${field}]`,
			);
			switch (action.type) {
				case "notify":
					results.push({
					...
					if (context.taskId) {
						...
						if (!ALLOWED_FIELDS.includes(action.field)) {
							results.push({
								type: "update_task",
								success: false,
								error: `Invalid field: ${action.field}`,
							});
							break;
						}
						await db
							.prepare(
								`UPDATE tasks SET ${action.field} = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
							)
				...
				case "send_message":
					await db
						.prepare(
							"INSERT INTO messages (channel, sender_type, sender_id, content) VALUES (?, 'agent', 0, ?)",
						)
				...
				case "create_task":
					await db
						.prepare(
							"INSERT INTO tasks (title, description, priority, department_id, created_by) VALUES (?, ?, ?, ?, 1)",
						)
						.run(
							message,
							action.description || "",
							action.priority || "medium",
							action.department_id || null,
						);
...
async function processTrigger(triggerName, context = {}) {
	...
	const rows = await db
		.prepare('SELECT * FROM workflows WHERE enabled = true AND "trigger" = ?')
		.all(triggerName);
	...
	for (const wf of matchedWorkflows) {
		if (evaluateConditions(wf.conditions, context)) {
			...
			await db
				.prepare(
					"UPDATE workflows SET runs = runs + 1, last_run = CURRENT_TIMESTAMP WHERE id = ?",
				)
				.run(wf.id);
			await db
				.prepare(
					'INSERT INTO workflow_logs (workflow_id, "trigger", context, results) VALUES (?, ?, ?, ?)',
				)
				.run(
					wf.id,
					triggerName,
	...
	const logCountRow = await db
		.prepare("SELECT COUNT(*) as c FROM workflow_logs")
		.get();
	const logCount = logCountRow ? logCountRow.c || logCountRow.count || 0 : 0;
	if (logCount > 1000)
		await db
			.prepare(
				"DELETE FROM workflow_logs WHERE id IN (SELECT id FROM workflow_logs ORDER BY created_at ASC LIMIT ?)",
			)
			.run(logCount - 1000);
	return results;
}
...
async function getExecutionLog(limit = 20) {
	const rows = await db
		.prepare(
			"SELECT wl.*, w.name as workflow_name FROM workflow_logs wl LEFT JOIN workflows w ON wl.workflow_id = w.id ORDER BY wl.created_at DESC LIMIT ?",
		)
		.all(limit);
	return rows.map((row) => ({
		...row,
...
async function getWorkflowStats() {
	...
	const enabledRow = await db
		.prepare("SELECT COUNT(*) as c FROM workflows WHERE enabled = true")
		.get();
	const enabled = enabledRow ? enabledRow.c || enabledRow.count || 0 : 0;
	const runsRow = await db
		.prepare("SELECT COALESCE(SUM(runs), 0) as s FROM workflows")
		.get();
	...
	return {
		...
		totalRuns,
		recentExecutions: await getExecutionLog(5),
	};
}
