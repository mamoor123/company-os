...
function seedWorkflows() {
  ...
  // Seeding happens synchronously at module load — use _db for SQLite direct access
  // This is safe because seedWorkflows only runs at startup
}

// For synchronous service code, unwrap the Promise
// by using a sync-aware helper
async function dbRun(sql, ...params) { return db.prepare(sql).run(...params); }
async function dbGet(sql, ...params) { return db.prepare(sql).get(...params); }
async function dbAll(sql, ...params) { return db.prepare(sql).all(...params); }

// Seed on load (use direct sync access for SQLite)
try {
  const row = db.prepare('SELECT COUNT(*) as c FROM workflows').get();
  // Handle both sync (SQLite) and thenable (PG) results
