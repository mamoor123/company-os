...
async function executeTask(taskId) {
	const task = await db
		.prepare(
			`SELECT t.*, a.name as agent_name, a.system_prompt, a.model, a.department_id as agent_dept_id FROM tasks t JOIN agents a ON t.assigned_agent_id = a.id WHERE t.id = ?`,
		)
		.get(taskId);
	...
	const comments = await db
		.prepare(
			`SELECT c.*, u.name as user_name, a.name as agent_name FROM task_comments c LEFT JOIN users u ON c.user_id = u.id LEFT JOIN agents a ON c.agent_id = a.id WHERE c.task_id = ? ORDER BY c.created_at DESC LIMIT 5`,
		)
		.all(taskId);
	if (comments.length > 0) {
		...
		comments.reverse().forEach((c) => {
			contextParts.push(
				`- ${c.user_name || c.agent_name || "Unknown"}: ${c.content.slice(0, 200)}`,
			);
		});
	}

	console.log(
		`🤖 Agent "${task.agent_name}" executing task #${taskId}: ${task.title}`,
	);
	await db
		.prepare(
...
async function agentDelegate(fromAgentId, toAgentId, message, taskId = null) {
	...
	const response = await callLLM(
		...
		`Delegation from ${fromAgent.name}:\n${message}`,
		toAgent.model,
	);
