...
async function checkSchedules() {
	...
	try {
		schedules = await db
			.prepare(
				"SELECT * FROM scheduled_tasks WHERE enabled = true AND (next_run IS NULL OR next_run <= ?)",
			)
			.all(now);
	} catch (err) {
	...
	for (const schedule of schedules) {
		...
		await db
			.prepare(
				"UPDATE scheduled_tasks SET run_count = run_count + 1, last_run = ?, next_run = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
			)
...
async function executeSchedule(schedule) {
	...
	const result = await db
		.prepare(
			"INSERT INTO tasks (title, description, priority, department_id, assigned_agent_id, created_by, status) VALUES (?, ?, ?, ?, ?, 1, 'pending')",
		)
		.run(
			schedule.name,
...
function calculateNextRun(schedule) {
	...
	switch (schedule.type) {
		case "interval":
			return new Date(
				now.getTime() + (schedule.interval_minutes || 60) * 60 * 1000,
			).toISOString();
		case "daily": {
...
async function createSchedule(data) {
	...
	const result = await db
		.prepare(
			"INSERT INTO scheduled_tasks (name, description, type, time, day_of_week, interval_minutes, agent_id, department_id, priority, next_run) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		)
		.run(
			data.name,
			data.description || "",
...
async function updateSchedule(id, data) {
	...
	if (
		...
		params.push(calculateNextRun({ ...existing, ...data }));
	}
...
async function deleteSchedule(id) {
	await db.prepare("DELETE FROM scheduled_tasks WHERE id = ?").run(id);
	return true;
}
...
async function toggleSchedule(id) {
	...
	await db
		.prepare(
			"UPDATE scheduled_tasks SET enabled = ?, next_run = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		)
...
async function getStatus() {
	...
	try {
		...
		const enabledRow = await db
			.prepare("SELECT COUNT(*) as c FROM scheduled_tasks WHERE enabled = true")
			.get();
