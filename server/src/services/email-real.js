...
function saveDraft({ to, subject, body, userId }) {
  const result = db.prepare("INSERT INTO emails (from_addr, to_addr, subject, body, folder, read, starred, user_id) VALUES (?, ?, ?, ?, 'drafts', 1, 0, ?)").run('admin@company-os.local', to || '', subject || '(no subject)', body || '', userId || null);
