...
function getSmtpTransport() {
	...
	console.log(
		`📧 SMTP transport configured: ${process.env.SMTP_HOST}:${process.env.SMTP_PORT || 587}`,
	);
...
function startImapPolling() {
	...
	const pollIntervalMs = parseInt(
		process.env.IMAP_POLL_INTERVAL_MS || "60000",
		10,
	);
	console.log(
		`📧 IMAP polling started: ${process.env.IMAP_HOST} (every ${pollIntervalMs / 1000}s)`,
	);
	...
	fetchNewEmails().catch((err) =>
		console.error("📧 IMAP initial fetch failed:", err.message),
	);
	...
	try {
		...
		try {
			const lastEmail = await db
				.prepare("SELECT MAX(uid) as max_uid FROM emails WHERE uid IS NOT NULL")
				.get();
			...
			for await (const message of imapClient.fetch(`${lastUid + 1}:*`, {
				...
				try {
					...
					const result = await db
						.prepare(
							"INSERT INTO emails (from_addr, to_addr, subject, body, folder, uid, message_id, date) VALUES (?, ?, ?, ?, 'inbox', ?, ?, ?)",
						)
						.run(
							parsed.from?.text || "",
							parsed.to?.text || "",
...
try {
	...
	const doSeed = (row) => {
		...
		const demo = [
			...
			{
				from_addr: "support@vendor.io",
				to_addr: "admin@company.com",
				subject: "Your API key has been renewed",
				body: "Your API key has been renewed until December 2026.\n\nNo action required.",
				folder: "inbox",
				read: true,
				starred: false,
			...
			{
				from_addr: "newsletter@industry.com",
				to_addr: "marketing@company.com",
		...
		const insert = db.prepare(
			"INSERT INTO emails (from_addr, to_addr, subject, body, folder, read, starred, labels, date) VALUES (@from_addr, @to_addr, @subject, @body, @folder, @read, @starred, @labels, @date)",
		);
		for (const email of demo) insert.run(email);
		console.log("✅ Seeded demo emails");
	};
	if (countRow && typeof countRow.then === "function") {
		countRow.then(doSeed).catch(() => {});
	} else {
		doSeed(countRow);
	}
} catch (_e) {
...
async function getEmail(id) {
	return rowToEmail(
		await db.prepare("SELECT * FROM emails WHERE id = ?").get(id),
	);
}
async function markRead(id) {
	await db.prepare("UPDATE emails SET read = true WHERE id = ?").run(id);
	return getEmail(id);
}
async function toggleStar(id) {
	await db
		.prepare("UPDATE emails SET starred = NOT starred WHERE id = ?")
		.run(id);
	return getEmail(id);
}
async function moveToFolder(id, folder) {
	await db.prepare("UPDATE emails SET folder = ? WHERE id = ?").run(folder, id);
	return getEmail(id);
}
...
async function sendEmail({ to, subject, body, inReplyTo, userId }) {
	...
	if (transport) {
		try {
			await transport.sendMail({
				from: process.env.SMTP_FROM || "no-reply@hiveops.local",
				to,
				subject,
				text: body || "",
			});
	...
	const result = await db
		.prepare(
			"INSERT INTO emails (from_addr, to_addr, subject, body, folder, read, starred, in_reply_to, user_id) VALUES (?, ?, ?, ?, 'sent', true, false, ?, ?)",
		)
		.run(
			process.env.SMTP_FROM || "admin@hiveops.local",
			to,
			subject,
			body || "",
			inReplyTo || null,
...
async function saveDraft({ to, subject, body, userId }) {
	const result = await db
		.prepare(
			"INSERT INTO emails (from_addr, to_addr, subject, body, folder, read, starred, user_id) VALUES (?, ?, ?, ?, 'drafts', true, false, ?)",
		)
		.run(
			"admin@hiveops.local",
			to || "",
...
async function getEmailStats() {
	const inboxRow = await db
		.prepare("SELECT COUNT(*) as c FROM emails WHERE folder = 'inbox'")
		.get();
	const unreadRow = await db
		.prepare(
			"SELECT COUNT(*) as c FROM emails WHERE folder = 'inbox' AND read = false",
		)
		.get();
	const sentRow = await db
		.prepare("SELECT COUNT(*) as c FROM emails WHERE folder = 'sent'")
		.get();
	const draftsRow = await db
		.prepare("SELECT COUNT(*) as c FROM emails WHERE folder = 'drafts'")
		.get();
	const starredRow = await db
		.prepare("SELECT COUNT(*) as c FROM emails WHERE starred = true")
		.get();
	return {
...
async function generateReply(emailId, instructions = "") {
	...
	if (process.env.LLM_API_KEY) {
		...
		return callLLM(
			`You are a professional email assistant. Draft a concise, helpful reply. ${instructions ? `Additional instructions: ${instructions}` : ""}`,
