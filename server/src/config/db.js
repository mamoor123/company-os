	...
	function prepare(sql) {
		...
		return {
			...
			get(...params) {
				const convertedParams = convertParams(params, namedParams, sql);
				return pool
					.query(transformedSql, convertedParams)
					.then((r) => coerceRow(r.rows[0]) || undefined);
			},
			all(...params) {
				const convertedParams = convertParams(params, namedParams, sql);
				return pool
					.query(transformedSql, convertedParams)
					.then((r) => coerceRows(r.rows));
			},
		};
	}
...
function createSqliteAdapter() {
	...
	function prepare(sql) {
		...
		return {
			...
			get(...params) {
				return stmt.get(...mapParams(params));
			},
			all(...params) {
